...
@php
$grupoLabels = [
    'Personas y Redes' => ['icon' => 'bi-people-fill'],
    'Procesos' => ['icon' => 'bi-mortarboard-fill'],
    'Finanzas' => ['icon' => 'bi-cash-coin'],
    'Sistema' => ['icon' => 'bi-shield-lock-fill'],
];
@endphp

<div class="pagetitle d-flex justify-content-between align-items-center flex-wrap gap-2">
    <div>
        <h1 class="mb-0"><i class="bi bi-toggles me-2"></i>{{ $rol->nombre }}</h1>
...
<form action="{{ route('modulos-rol.update', $rol->id) }}" method="POST">
    @csrf @method('PUT')

    <div class="row g-3">
        @foreach($modulos as $grupo => $items)
        @php $info = $grupoLabels[$grupo] ?? ['icon' => 'bi-grid']; @endphp
        <div class="col-12 col-md-6">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-header bg-light d-flex justify-content-between align-items-center py-2">
                    <span class="fw-semibold">
                        <i class="bi {{ $info['icon'] }} me-2 text-primary"></i>{{ $grupo ?? 'General' }}
                    </span>
                    <button type="button" class="btn btn-link btn-sm p-0 text-muted toggle-grupo" data-grupo="{{ $loop->index }}">
                        Sel. todos
                    </button>
                </div>
                <div class="card-body py-2">
                    ...
                    <div class="form-check py-1 border-bottom">
                        <input class="form-check-input modulo-check grupo-{{ $loop->parent->index }}"
                               type="checkbox"
                               name="modulos[]"
                               value="{{ $modulo->id }}"
                               id="mod_{{ $modulo->id }}"
